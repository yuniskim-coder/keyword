...
def main():
    ...
    # 탭 생성
    tab1, tab2, tab3, tab4 = st.tabs(["🎯 순위 검색", "🔗 연관키워드 찾기", "📊 연관키워드 순위", "🏆 순위 랭킹"])
    ...
    with tab2:
        ...
        if related_search_button and base_keyword:
            with st.spinner(f"'{base_keyword}' 연관키워드 검색 중..."):
            ...
            if related_keywords:
                ...
                st.info("� 이제 '📊 연관키워드 순위' 탭으로 이동하여 원하는 키워드들의 순위를 확인하세요!")
    ...
    with tab3:
        ...
        with st.sidebar:
            ...
            st.markdown("- 검색 결과가 없는 키워드는 별도 표시됩니다")
        ...
        if 'found_related_keywords' in st.session_state and st.session_state.found_related_keywords:
            ...
            with col1:
                ...
                # 연관키워드를 체크박스로 표시 (3열 구조)
                cols = st.columns(3)
                selected_keywords = []
                
                for i, keyword in enumerate(st.session_state.found_related_keywords):
                    with cols[i % 3]:
                        if st.checkbox(f"{keyword}", key=f"rank_related_{i}"):
        ...
        if ranking_search_button and ranking_keywords_input and ranking_mall_name:
            ...
            if include_related:
                ...
                for i, keyword in enumerate(ranking_keywords):
                    ...
                    all_search_keywords.extend(related_keywords[:3])  # 각 키워드당 상위 3개 연관키워드
                    time.sleep(0.1)
                
                related_progress.progress(1.0)
                st.success(f"✅ 총 {len(all_search_keywords)}개 키워드로 검색합니다!")
